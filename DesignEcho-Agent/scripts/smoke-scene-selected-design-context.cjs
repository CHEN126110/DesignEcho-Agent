...
const reportJsonPath = path.join(__dirname, '..', 'tmp', 'scene-selected-design-context-smoke.json');
const reportMdPath = path.join(__dirname, '..', 'tmp', 'scene-selected-design-context-smoke.md');
...
function writeReport(report) {
  ...
  const lines = [
    '# Scene Selected Design Context Smoke',
    '',
    `- Endpoint: ${endpoint}`,
...
async function main() {
  ...
  if (systemStatus?.pluginConnected !== true) {
    ...
    writeReport(report);
    console.log(`[smoke-scene-selected-design-context] skipped: ${report.skipReason}`);
    return;
  }

  try {
    const toolPayload = await callTool('scene.get_selected_design_context', {
      includeText: true,
      includeDetailContext: false,
      relationLimit: 6
    });
    report.tool.payload = toolPayload;
    if (isUsableDesignContext(toolPayload)) {
    ...
    const resourcePayload = await readResource('designecho://scene/selected-design-context');
    report.resource.payload = resourcePayload;
    if (isUsableDesignContext(resourcePayload)) {
    ...
    report.skipped = true;
    report.skipReason = 'Photoshop bridge is connected but not responding to tool calls';
    writeReport(report);
    console.log(`[smoke-scene-selected-design-context] skipped: ${report.skipReason}`);
    return;
  }

  const promptPayload = await getPrompt('selected-design-context-audit', {});
  report.prompt.success = Array.isArray(promptPayload?.messages) && promptPayload.messages.length > 0;

  writeReport(report);

  console.log(`[smoke-scene-selected-design-context] report: ${reportJsonPath}`);
  ...
  console.log('PASS: scene selected design context smoke test');
}
