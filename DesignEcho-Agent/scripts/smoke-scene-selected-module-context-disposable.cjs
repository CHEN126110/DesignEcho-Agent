...
const JSON_OUT = path.join(TMP_DIR, 'scene-selected-module-context-disposable.json');
const MD_OUT = path.join(TMP_DIR, 'scene-selected-module-context-disposable.md');
...
function renderMarkdown(report) {
  const lines = [];
  lines.push('# Scene Selected Module Context Disposable Smoke');
  lines.push('');
...
function isUsableModuleContext(payload) {
  return payload?.success === true
    && payload?.context?.selectedElementLayerId > 0
    && payload?.context?.module?.id;
}
...
async function main() {
  ...
  if (openDocuments.length === 0) {
    ...
    fs.writeFileSync(MD_OUT, `# Scene Selected Module Context Disposable Smoke\n\n- Skipped: ${skippedReport.skipReason}\n`);
    console.log(`[smoke-scene-selected-module-context-disposable] skipped: ${skippedReport.skipReason}`);
    return;
  }

  const stamp = Date.now();
  const docName = `Scene-Module-Smoke-${stamp}`;
  let createdDoc = null;
  ...
  try {
    ...
    createdText = await callTool('photoshop.tools.call', {
      ...
      arguments: {
        content: 'Module Context Smoke',
        x: 60,
        y: 160,
        name: 'Module Smoke Text',
        fontSize: 24
      }
    });
    ...
    const textContext = await callTool('scene.get_selected_module_context', {
      relationLimit: 6
    });
    ...
    const shapeContext = await callTool('scene.get_selected_module_context', {
      relationLimit: 6
    });

    const scenarios = [
      {
        name: 'selected-text-layer-module-context',
        expected: 'Selected text layer should resolve to a visual module and include module relations.',
        outcome: isUsableModuleContext(textContext) ? 'pass' : 'fail',
        payload: textContext
      },
      {
        name: 'selected-shape-layer-module-context',
        expected: 'Selected shape layer should resolve to a visual module and include module relations.',
        outcome: isUsableModuleContext(shapeContext) ? 'pass' : 'fail',
