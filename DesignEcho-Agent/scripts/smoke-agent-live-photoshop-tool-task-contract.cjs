...
const RUNNER = path.join(ROOT, 'scripts', 'run-agent-live-photoshop-tool-task.cjs');
...
function main() {
  ...
  assert(
    pkg.scripts?.['smoke:agent:live-photoshop-tool-task'] === 'node scripts/run-agent-live-photoshop-tool-task.cjs',
    'package script smoke:agent:live-photoshop-tool-task must point to the live Agent Photoshop runner.'
  );
  assert(
    pkg.scripts?.['smoke:agent:live-photoshop-tool-task:contract'] === 'node scripts/smoke-agent-live-photoshop-tool-task-contract.cjs',
    'package script smoke:agent:live-photoshop-tool-task:contract must point to this contract smoke.'
  );
  assert(fs.existsSync(RUNNER), 'live Agent Photoshop runner is missing.', { RUNNER });
