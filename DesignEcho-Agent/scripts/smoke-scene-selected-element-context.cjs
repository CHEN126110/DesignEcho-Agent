...
const reportJsonPath = path.join(__dirname, '..', 'tmp', 'scene-selected-element-context-smoke.json');
const reportMdPath = path.join(__dirname, '..', 'tmp', 'scene-selected-element-context-smoke.md');
...
function writeReport(report) {
  ...
  const lines = [
    '# Scene Selected Element Context Smoke',
    '',
    `- Endpoint: ${endpoint}`,
...
async function main() {
  ...
  if (systemStatus?.pluginConnected !== true) {
    ...
    writeReport(report);
    console.log(`[smoke-scene-selected-element-context] skipped: ${report.skipReason}`);
    return;
  }

  const toolPayload = await callTool('scene.get_selected_element_context', {
    includeText: true,
  ...
  const resourcePayload = await readResource('designecho://scene/selected-element-context');
  ...
  const promptPayload = await getPrompt('selected-element-context-audit', {});
  report.prompt.success = Array.isArray(promptPayload?.messages) && promptPayload.messages.length > 0;

  writeReport(report);

  console.log(`[smoke-scene-selected-element-context] report: ${reportJsonPath}`);
  ...
  console.log('PASS: scene selected element context smoke test');
}
