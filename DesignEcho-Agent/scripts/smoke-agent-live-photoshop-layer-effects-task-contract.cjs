...
const RUNNER = path.join(ROOT, 'scripts', 'run-agent-live-photoshop-layer-effects-task.cjs');
...
function main() {
  ...
  assert(
    pkg.scripts?.['smoke:agent:live-photoshop-layer-effects-task'] === 'node scripts/run-agent-live-photoshop-layer-effects-task.cjs',
    'package script smoke:agent:live-photoshop-layer-effects-task must point to the live Agent Photoshop layer/effects runner.'
  );
  assert(
    pkg.scripts?.['smoke:agent:live-photoshop-layer-effects-task:contract'] === 'node scripts/smoke-agent-live-photoshop-layer-effects-task-contract.cjs',
    'package script smoke:agent:live-photoshop-layer-effects-task:contract must point to this contract smoke.'
  );
  assert(fs.existsSync(RUNNER), 'live Agent Photoshop layer/effects runner is missing.', { RUNNER });
  ...
  assert(source.includes('setLayerOpacity'), 'runner must cover layer opacity writes.');
  assert(source.includes('addStroke'), 'runner must cover real Photoshop stroke effects.');
  assert(source.includes('addDropShadow'), 'runner must cover real Photoshop drop shadow effects.');
  assert(source.includes('hasMatchingLayerArg('), 'runner must require explicit layerId targeting evidence.');
  ...
  console.log(JSON.stringify({
    ...
    checks: [
      ...
      'runner covers explicit layerId opacity/effects writes',
